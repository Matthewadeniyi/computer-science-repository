<?php 
  //$db = new mysqli('localhost','root','','matthew');
  //include_once('config.php');

  function win_hash($length){
    return substr(str_shuffle(str_repeat('123456789')),0,$length);
  }
  ...
  function win_hashs($length){
	...
	function userName($id,$col=''){
	    global $db;
	    $sql = $db->query("SELECT * FROM user WHERE id='$id' OR sn='$id' ");
        if(mysqli_num_rows($sql)==0){return '';}
	    $row = $sql->fetch_assoc();
...
// $arr = ['firstname'=>'Olwale','surname'=>'Kingsley','phone'=>'08032318588'];
// $arr2 = ['admin'=>22,'time'=>time()];
// $arrx = array_merge($arr,$arr2);
// unset($arrx['time']);
// $var = extract($_POST);
// $rr;
// dbInsert('user',$arr);
...
class profile1{
  ...
  public function signUp(){
    ...
    $array = [
      ...
      'othername' => sanitize($othername),
      'username' => sanitize($username),
      'email' => sanitize($email),
      'password' => password_hash($password, PASSWORD_BCRYPT),
      'referral_id'=> sanitize($referral),
      'id'=> win_hashs(6)
    ];

    $query = $db->query("SELECT * FROM user WHERE email='{$array['email']}' OR username='{$array['username']}' ");

    if(mysqli_num_rows($query) > 0){
      $rows = mysqli_fetch_assoc($query);
      if($rows['email'] == $array['email']){
        echo "Email Already Exists!";
      }elseif($rows['username'] == $array['username']){
        echo "Username Already Taken!";
      }
    
      return;
    }
      // insert into table with the dbInsert function
    $sql = dbInsert('user', $array);
  ...
  public function login(){
    ...
    $sql = sqL1('user','username',$array['username']);
    if($sql !=1 ){echo '<h2 class="text-center mb-4">Invalid Login</h2>'; exit;}
    $sql = $db->query("SELECT * FROM user WHERE username='{$array['username']}'");
    $rows = mysqli_fetch_assoc($sql);
    if(password_verify($array['password'],$rows['password'])){
      session_start();
      $_SESSION['user_id'] = $rows['sn']; // 'user_id' is the unique identifier
      $_SESSION['firstname'] = $rows['firstname']; // assigning values of the user into table
      $_SESSION['othername'] = $rows['othername']; // assigning values of the user into table
      $_SESSION['username'] = $rows['username'];
      header('Location:index.php');
      exit;
    }else{
      echo '<h2 class="text-center mb-4">Invalid Login</h2>';
    }
    return ;
  }
}
